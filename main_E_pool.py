...
from paired_net import PairedSimplifiedMLP, PairedSimplifiedGCN, PairPooling
...
def test(epoch, final=False):
    ...
    ranking_out_dirs = Path(data_folder) / Path('eval/pairwise_pooling')
    if not os.path.exists(ranking_out_dirs):
        os.makedirs(ranking_out_dirs)
    ranking_out_name = f"pairwise_pooling_epoch{epoch}_batch{batch_size}_gc{num_gcn_layers}_mlp{num_mlp_layers}_lr{learning_rate}_{activation}_{readout}.txt"
    ...
    if os.path.exists(eval_excutable):  
        ...
        print(f'pairwise pooling ndcg_20: {ndcg_20}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--param', type=str, default='local:pairwise.yaml')
    ...
    model =  PairPooling(
        PairedSimplifiedGCN(dataset_num_features, hidden_dim, num_gcn_layers, get_activation(activation), readout),
        PairedSimplifiedMLP(num_gcn_layers * (hidden_dim * 2), hidden_dim, num_mlp_layers, get_activation(activation)),
    ).to(device)
    ...
    print(f'E pool final_ndcg_20: {ndcg_20}')
    print(f'E pool best_ndcg_20: {best_ndcg_20}')
    print(f'E pool best_params: {best_params}')