...
from path_net import PathSimplifiedMLP, PathSimplifiedGCN, PathPooling
...
def test(epoch, final=False):
    ...
    ranking_out_dirs = Path(data_folder) / Path('eval/path_pooling')
    if not os.path.exists(ranking_out_dirs):
        os.makedirs(ranking_out_dirs)
    ranking_out_name = f"path_pooling_epoch{epoch}_batch{batch_size}_gc{num_gcn_layers}_mlp{num_mlp_layers}_lr{learning_rate}_{activation}_{readout}.txt"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--param', type=str, default='local:path.yaml')
    ...
    model =  PathPooling(
        PathSimplifiedGCN(dataset_num_features, hidden_dim, num_gcn_layers, get_activation(activation), readout),
        PathSimplifiedMLP(num_gcn_layers * hidden_dim, hidden_dim, num_mlp_layers, get_activation(activation)),
    ).to(device)
    ...
    print(f'RW pool final_ndcg_20: {ndcg_20}')
    print(f'RW pool best_ndcg_20: {best_ndcg_20}')
    print(f'RW pool best_params: {best_params}')