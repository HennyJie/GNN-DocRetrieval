...
from net import MLP, GCN, Net, GAT
...
def test(epoch, final=False):
    ...
    ranking_out_dirs = Path(data_folder) / Path('eval/gat')
    if not os.path.exists(ranking_out_dirs):
        os.makedirs(ranking_out_dirs)
    ranking_out_name = f"gat_epoch{epoch}_batch{batch_size}_gc{num_gcn_layers}_mlp{num_mlp_layers}_lr{learning_rate}_{activation}_{readout}.txt"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--param', type=str, default='local:gat.yaml')
    ...
    first_heads = param['first_heads']
    output_heads = param['output_heads']
    ...
    model =  Net(
        GAT(dataset_num_features, hidden_dim, num_gcn_layers, first_heads, output_heads, dropout, get_activation(activation), readout),
        MLP(hidden_dim * first_heads + (num_gcn_layers-1) * hidden_dim, hidden_dim, num_mlp_layers, get_activation(activation)),
    ).to(device)
    ...
    print(f'gat final_ndcg_20: {ndcg_20}')
    print(f'gat best_ndcg_20: {best_ndcg_20}')
    print(f'gat best_params: {best_params}')